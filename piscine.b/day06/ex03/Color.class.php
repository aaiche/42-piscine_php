...
Class Color {
	...
	private function _getRgb() 		{ return $this->_rgb; 			}
	private function _setRgb($rgb)	{ $this->_rgb = $rgb; 	return;	} 
	
	//
	// Used also when verbose
	// Permet de serialiser, ie, transformer une instance en une chaine de caracteres. 
	// Cette methode est tres pratique qd on veut debugger ou faire du log
	// L appel sera fera ainsi:
	// 		$instance = new Color(ainsi:
	// 				$instance = new Color(..)
	// 			puis, on appelle DIRECTEMENT l instance
	...
	//
	// method will check that kwargs is an array
	// on utilise le trick de python pour implementer la surcharge dans php
	//
	public function __construct( array $kwargs ) {
		if( (count($kwargs) == 1) && array_key_exists('rgb', $kwargs) ) {
				// get the rgb arg
				$this->_setRgb(intval($kwargs['rgb']));

				// set the attributes
				$this->setRed(($this->_getRgb() >> 16) & 0xff);
				$this->setGreen(($this->_getRgb() >> 8) & 0xff);
				$this->setBlue($this->_getRgb() & 0xff);
			...
			&& array_key_exists('blue', $kwargs) ) {
				...
				$this->_setRgb(($this->getRed() << 16 | $this->getGreen() << 8 | $this->getBlue()));
		} else {
			// invalid arguments
	...
	// On force le controle du type de parameter
